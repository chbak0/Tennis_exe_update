...
# <<<--- 분석용 Supabase 설정 (새로 추가된 부분) --->>>
# 🚨🚨🚨 아래 두 값을 질문자님의 Supabase 프로젝트 값으로 꼭 변경해주세요! 🚨🚨🚨
...
APP_VERSION = "1.0.0" # 나중에 앱 버전을 올릴 때 이 숫자만 바꿔주면 됩니다.

class AnalyticsLogger:
    """분석 데이터를 새로운 Supabase DB로 전송하는 클래스"""
...
def decrypt_password(encrypted_password: str) -> str:
    ...
    except Exception as e: logging.error(f"비밀번호 복호화 실패: {e}"); return ""
...
class TennisBookingGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("송도 테니스 예약 자동화 (Final)")
        self.time_offset = 0
        
        # <<<--- 분석 로거 인스턴스 생성 (추가된 부분) --->>>
        ...
        # <<<--- machine_id 및 세션 시작 시간 기록 (추가된 부분) --->>>
        self.machine_id = self.load_or_create_machine_id()
        self.session_start_time = datetime.now() # 동기화 전 시간으로 우선 기록
    ...
    # <<<--- machine_id 생성/로드 메소드 (새로 추가된 부분) --->>>
        ...
        if not machine_id:
            ...
            try:
                with open(self.config_file, 'w', encoding='utf-8') as f:
                    # 기존 설정이 있다면 유지하면서 machine_id만 추가/수정
                    # self.save_config()를 직접 호출하면 암호가 다시 암호화되므로 부분적으로 처리
                    # 하지만, 여기서는 간결함을 위해 machine_id만 있는 config를 저장.
                    # load_config에서 나머지 정보가 덮어쓰므로 괜찮음.
                    json.dump({'machine_id': machine_id}, f, indent=2)
                self.log_message(f"새로운 Machine ID 생성: {machine_id}")
            except IOError as e:
    ...
    def _sync_time_worker(self):
        try:
            ...
            self.session_start_time = self.get_synced_time() # 동기화된 시간으로 다시 기록
    ...
    def _load_reservations_worker(self):
        ...
        if not reservations_details or not isinstance(reservations_details, list):
            self.root.after(0, self.update_my_bookings_tree, []); return
        payment_statuses = self.booking_api.get_payment_statuses()
        enriched_reservations = []
        for res_detail in reservations_details:
            if not isinstance(res_detail, dict): continue
            res_id = res_detail.get('id')
            if res_id: res_detail['payment_status'] = payment_statuses.get(res_id)
    ...
    def update_my_bookings_tree(self, reservations: List[Dict[str, Any]]):
        ...
        if not reservations: 
            ...
            self.analytics_logger.log_event(
                ...
                event_data={"reservation_count": 0, "reservations": []} # 상세 내역을 담을 빈 리스트 추가
            )
            return

        items_added = 0
        loaded_reservations_details = [] # 상세 내역을 담을 리스트 생성
    ...
    def show_auto_add_dialog(self, day_type: str):
        ...
        def update_date_list():
            for w in date_check_frame.winfo_children():
                w.destroy()
        ...
        def add_targets():
            added, dates, times, courts = 0, [d for d,v in vars_dict['dates'].items() if v.get()], [t for t,v in vars_dict['times'].items() if v.get()], [c for c,v in vars_dict['courts'].items() if v.get()]
    ...
    def show_booking_time_setting(self):
        ...
        temp_vars = {
            'year': tk.IntVar(value=self.booking_year_var.get()),
            'month': tk.IntVar(value=self.booking_month_var.get()),
            'day': tk.IntVar(value=self.booking_day_var.get()),
            'hour': tk.IntVar(value=self.booking_hour_var.get()),
            'minute': tk.IntVar(value=self.booking_minute_var.get()),
            'second': tk.IntVar(value=self.booking_second_var.get())
        }
        ...
        def apply_settings():
            for key, var in temp_vars.items():
                getattr(self, f"booking_{key}_var").set(var.get())
            self.calculate_booking_time()
        ...
        def reset_to_default():
            ...
            if now.day <= booking_day_setting:
                target_date = now
            else:
                target_date = now.replace(day=28) + timedelta(days=4)
    ...
    def save_config(self):
        config = {
            ...
            'machine_id': self.machine_id, # machine_id도 함께 저장
            'booking_targets': self.booking_targets,
            'booking_time': {
                'year': self.booking_year_var.get(), 'month': self.booking_month_var.get(), 
    ...
    def load_config(self):
        ...
        try:
            ...
            self.booking_targets = config.get('booking_targets', []); [self.targets_tree.delete(i) for i in self.targets_tree.get_children()]
            ...
            if time_settings:
                self.booking_year_var.set(time_settings.get('year')); self.booking_month_var.set(time_settings.get('month')); self.booking_day_var.set(time_settings.get('day')); self.booking_hour_var.set(time_settings.get('hour')); self.booking_minute_var.set(time_settings.get('minute')); self.booking_second_var.set(time_settings.get('second'))
    ...
    def auto_set_default_booking_time(self):
        ...
        if now.day <= booking_day_setting: target_date = now
        else: target_date = now.replace(day=28) + timedelta(days=4)
        self.booking_year_var.set(target_date.year); self.booking_month_var.set(target_date.month); self.booking_day_var.set(booking_day_setting); self.booking_hour_var.set(10); self.booking_minute_var.set(0); self.booking_second_var.set(0)
    ...
    def start_booking(self):
        ...
        self.is_booking_active=True; self.start_button.config(state=tk.DISABLED); self.stop_button.config(state=tk.NORMAL); self.log_message("예약 시작!")
    ...
    async def booking_worker_async(self, targets: List[Dict[str, Any]]):
                    ...
                    if tasks:
                        ...
                        for i, res in enumerate(results):
                            ...
                            if t_key not in successful:
                                ...
                                self.root.after(0, self.log_message, f"✅ [{t_date_str} {t_court_str}번 {t_time_str}] 성공!" if res['success'] else f"❌ [{t_date_str} {t_court_str}번 {t_time_str}] 실패: {res['message']}")
                                if res['success']:
                                    successful.add(t_key)
                                else:
                                    failures.append({
                                        "target": {"date": t_date_str, "court": int(t_court_str), "time": t_time_str},
                                        "reason": res['message']
                                    })
                except Exception as e:
        ...
        # <<<--- 이 부분이 추가/수정되었습니다 --->>>
        # 예약 경쟁이 끝난 후, 사용자가 직접 중지했는지 확인합니다.
        if not self.is_booking_active:
            # 아직 성공하지 못한 모든 목표를 찾습니다.
            for t in targets:
                t_key = f"{t['date']}|{t['court']}|{t['time']}"
                if t_key not in successful:
                    # 실패 목록에 '사용자 중지' 사유로 추가합니다.
                    failures.append({
                        "target": t,
                        "reason": "사용자가 직접 중지"
                    })
        ...
        summary_data = {
            ...
            "successful_bookings": successful_list,
...
def main():
    ...
    def on_closing():
        ...
        if app.is_logged_in:
            ...
            time.sleep(0.5) # 로그가 전송될 시간을 잠시 줍니다.
